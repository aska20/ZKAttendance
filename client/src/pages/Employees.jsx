...
// ── EmployeeDetailModal ───────────────────────────────────────────────────────
function EmployeeDetailModal({ emp, deptName, onClose, onEdit, onCalendar, onToggleLogin, onChangeRole }) {
  if (!emp) return null
    ...
    <Modal title="Employee Details" onClose={onClose}>
      <div className="flex flex-col items-center gap-3 pb-4">
        ...
        <div className="text-center">
          ...
          {emp.title && <div className="text-sm text-sky-600 font-medium mt-0.5">{emp.title}</div>}
        </div>
      </div>

      <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3.5 text-sm border-t border-slate-100 pt-4">
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Biometric ID</div>
          <div className="font-mono font-medium text-slate-800">{emp.biometricUserId || '—'}</div>
        </div>
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Department</div>
          <div className="font-medium text-slate-800">{deptName(emp.departmentId)}</div>
        </div>
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Job Title</div>
          <div className="font-medium text-slate-800">{emp.title || '—'}</div>
        </div>
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Hire Date</div>
          <div className="font-medium text-slate-800">
            <FormattedDate date={emp.hireDate} dateBs={emp.hireDateBs} />
          </div>
        </div>
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Status</div>
          <div className="mt-0.5">
            {emp.isActive ? <Badge tone="green">Active</Badge> : <Badge tone="slate">Inactive</Badge>}
          </div>
        </div>
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Phone</div>
          <div className="font-medium text-slate-800">{emp.phoneNumber || '—'}</div>
        </div>
        <div className="col-span-1 sm:col-span-2">
          <div className="text-xs text-slate-400 mb-0.5">Email</div>
          <div className="font-medium text-slate-800 truncate">{emp.email || '—'}</div>
        </div>

        {/* Portal access is managed from the table column — not shown here */}
      </div>
      ...
      <div className="mt-6 flex flex-wrap justify-end gap-2 border-t border-slate-100 pt-4">
        ...
        {onCalendar && (
          ...
          >
            <MdCalendarMonth className="mr-1.5 inline-block h-4 w-4" /> Attendance Calendar
          </Button>
        )}
        <Button
          onClick={() => {
        ...
        >
          <FaEdit className="mr-1.5 inline-block h-3.5 w-3.5" /> Edit Employee
        </Button>
      </div>
    </Modal>
  )
